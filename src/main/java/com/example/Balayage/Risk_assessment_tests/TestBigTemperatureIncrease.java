...
public class TestBigTemperatureIncrease {
    ...
    public static String checkBigTemperatureIncrease(List<Map<String, String>> list_upcoming_weathers) {
        int max_temp = Integer.valueOf(list_upcoming_weathers.get(0).get("max_temp_c"));
        int highest_temp_index = 0;
        //Get highest upcoming temperature
        for(int i = 0; i < list_upcoming_weathers.size();i++){
            if (Integer.valueOf(list_upcoming_weathers.get(i).get("max_temp_c")) > max_temp) {
                max_temp = Integer.valueOf(list_upcoming_weathers.get(i).get("max_temp_c"));
                highest_temp_index = i + 2;
            }
        }

        if (max_temp - Integer.valueOf(list_upcoming_weathers.get(0).get("max_temp_c")) > 3)
